...
from app.pipelines.pdf_extraction.semantic_search import (
...
async def _resolve_pdf_bytes(file: UploadFile | None, url: str | None) -> tuple[bytes, str]:
...
async def extract_context(
    ...
    contents, source_name = await _resolve_pdf_bytes(file, url)
    logger.info(
...
@router.post("/extract/semantic_search", summary="Search a PDF document by semantic (embedding) similarity")
async def extract_context_semantic(
    file: UploadFile | None = File(None, description="PDF document to search"),
    url: str | None = Form(None, description="URL of a PDF document to search (alternative to file upload)"),
    query: str = Form(..., description="Natural-language question to search for"),
    top_k: int = Form(2, description="Number of top sections to return"),
    min_score: float = Form(0.0, description="Minimum cosine similarity threshold (0-1)"),
    max_pages: int | None = Form(
        None, description="Only process the first N pages of the PDF. Omit to process all pages."
    ),
):
    ...
    if not settings.OPENAI_API_KEY:
        raise HTTPException(status_code=503, detail="Semantic search is not configured (missing OPENAI_API_KEY).")

    request_start = time.perf_counter()
    contents, source_name = await _resolve_pdf_bytes(file, url)
    logger.info(
        f"Received /pdf/extract/semantic_search request: source={source_name!r}, query={query!r}, "
        f"top_k={top_k}, min_score={min_score}, max_pages={max_pages}"
    )
    ...
    try:
        ...
        logger.exception(f"Failed to process {source_name!r} for query {query!r}")
        raise HTTPException(status_code=502, detail=f"Semantic search failed: {e}")
    finally:
        os.unlink(tmp_path)
        logger.info(f"Cleaned up temp file {tmp_path}")
    ...
    if not results:
        ...
        return JSONResponse(
            ...
            content={
                ...
                "matches": [],
                "context": "",
                "message": "No sections matched the query above the similarity threshold.",
            },
        )
